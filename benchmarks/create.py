...
# Shared collections for both generators. This keeps content distribution
# and section/list generation comparable across tools.
BENCH_COLLECTIONS = ["blog", "notes", "tutorials"]
...
def scaffold_hugo():
    ...
    with open(os.path.join(root, "hugo.toml"), "w") as f:
        f.write(
            ...
            # Disable outputs not produced by Palya so both generators
            # render the same page types.
    ...
    for section in BENCH_COLLECTIONS:
...
def generate_posts():
    for i in range(1, NUM_POSTS + 1):
        ...
        # Use the same collection set for a comparable workload.
        collection = random.choice(BENCH_COLLECTIONS)
        body = gen_body(raw_title)

        # Palya
        p_dir = os.path.join(BENCH_DIR, "palya", "content", collection)
